...
Need to check that max number of neurons, input shape of encoder and output shape of decoder in general_deep_learning.py are set to 2 before running this script
"""
...
def main():
    ...
    if os.path.exists('deep_learning_unsupervised_qtl/best_unsup_model_by_qtl.keras'): # check for existence of the best clustering model
        
        print('The model has already been trained and saved on disk!')
        
        best_model=tf.keras.models.load_model('deep_learning_unsupervised_qtl/best_unsup_model_by_qtl.keras')
    
    elif os.path.exists('deep_learning_unsupervised_qtl/best_checkpoint.keras'): # check for existence of the best checkpoint
        
        print('The model has already been trained and saved on disk!')
        
        best_model=tf.keras.models.load_model('deep_learning_unsupervised_qtl/best_checkpoint.keras')

    else:
        
        # Perform search of the best hyperparameters
        
        hyperband_tuner=kt.Hyperband(MyUnsupervisedTaskTuning(), objective=kt.Objective('val_accuracy', 'max'), seed=2024, max_epochs=10, factor=2, hyperband_iterations=2, overwrite=True, directory='deep_learning_unsupervised_qtl', project_name='hyperband')
        
        checkpoint_cb = tf.keras.callbacks.ModelCheckpoint('deep_learning_unsupervised_qtl/best_checkpoint.keras', save_best_only=True)
        ...
        best_model.save('deep_learning_unsupervised_qtl/best_unsup_model_by_qtl.keras') # save it
