...
list_json_files=[os.path.join('json_data/', file) for file in os.listdir('json_data/') if 'pheno' in file]
    ...
    for el in json_contents[2].split(',{'):
        ...
        for (x,y) in enumerate(data):
            ...
            if not (strain_found==None):
                ...
                value_found1=re.search('value', data[x+1]) # trait value expected to be the next element
                ...
                try:
                    value_found2=re.search('value', data[x+2]) # or maybe the second next element
...
for jsonf in list_json_files:
    ...
    if contains_phenotype_data(jsonf):
        ...
        process_json_bimbam(jsonf, 'new_phenotype_file.csv')
