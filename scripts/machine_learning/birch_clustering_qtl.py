...
class Columns2Clustering(ModellingBirch):
    ...
    """
    Represent clustering task on only 2 columns obtained from transformation
    ...
    def perform_birch(self, reduced_features_valid, n_clusters):
        ...
        birch_clustering=Pipeline([('preprocessing_qtl', preprocessing_qtl), ('birch', Birch(n_clusters=n_clusters))]) # reperform clustering with the best number of clusters
        birch_clustering.fit(self.training)
        
        return birch_clustering
    
    
    def visualize_plot(plot_birch, birch_clustering, X_train, n_clusters, size=200):
        """
        ...
        plt.savefig(os.path.join(out_dir, f"Birch_clustering_result_by_qtl_{n_clusters}_clusters"))
    ...
    def annotate_plot(X, anno, type_anno, size=500):
        ...
        plt.savefig(os.path.join(out_dir, f"Birch_qtl_clustering_data_annotated_{type_anno}"), dpi=500)
...
def main():
    ...
    if os.path.exists('birch_clustering/birch_clustering_qtl.pkl'): # check if this has already been saved
