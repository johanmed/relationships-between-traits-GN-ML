...
NB: Need to make sure input shape of ANN in general_deep_learning.py is set to the right number of dimensions expected for QTL modeling -> 2
"""
...
def main():
    ...
    if os.path.exists('deep_learning_supervised_qtl/best_sup_model_by_qtl.keras'): # check for existence of the best clustering model
        
        print('The model has already been trained and saved on disk!')
        
        best_model=tf.keras.models.load_model('deep_learning_supervised_qtl/best_sup_model_by_qtl.keras')
    
    elif os.path.exists('deep_learning_supervised_qtl/best_checkpoint.keras'): # check for existence of the best checkpoint
        
        print('The model has already been trained and saved on disk!')
        
        best_model=tf.keras.models.load_model('deep_learning_supervised_qtl/best_checkpoint.keras')

    else:
        
        # Perform search of the best hyperparameters
        
        hyperband_tuner=kt.Hyperband(MySupervisedTaskTuning(), objective=kt.Objective('val_RootMeanSquaredError', 'min'), seed=2024, max_epochs=10, factor=2, hyperband_iterations=2, overwrite=True, directory='deep_learning_supervised_qtl', project_name='hyperband')
        
        checkpoint_cb = tf.keras.callbacks.ModelCheckpoint('deep_learning_supervised_qtl/best_checkpoint.keras', save_best_only=True)
        ...
        best_model.save('deep_learning_supervised_qtl/best_sup_model_by_qtl.keras') # save it
